...
double Arc::eq(double x,double yd){

    return (pow(x-this->focus->x(),2)/2*(this->focus->y()-yd))+((this->focus->y()+yd)/2);

}
QPoint Arc::GetSecondIntersection( Arc& second_arch, double line_pos){
    QPoint intersect_points(0,0);
            ...
            if (fabs(p1) <=0,000001) {
          intersect_points.setX(this->focus->x());  intersect_points.setY(this->eq(this->focus->x(),line_pos));
            } else {
...
QPoint Arc::GetFirstIntersection( Arc& second_arch, double line_pos){
    QPoint intersect_points(0,0);
            double p1 = 2 * (line_pos - this->focus->y());
            double p2 = 2 * (line_pos - second_arch.focus->y()); // is not 0.0, because line moved down
            if (fabs(p1) <=0,000001) {
